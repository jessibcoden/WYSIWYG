...
function addCeleb (name, birth, death, subTitle, image, bio) {
	var newCeleb = {name, birth, death, subTitle, image, bio}
...
addCeleb('Ada Lovelace', '1815', '1852', 'The First Computer Programmer', 'http://bygeekgirl.com/wp-content/uploads/2016/09/By-Geekgirl-Ada-Lovelace-Wikipedia-800x550.jpg', ['Introduced by her mentor to Charles Babbage, at age seventeen Ada Lovelace became interested in the "Analytical Engine", the invention the Mathematics professor was working on. Expanding on her interest of the machine, Ada Lovelace translated an article written by an Italian mathematician describing the computer-like machine. Impressed with her work, Charles Babbage asked Ada to expand on the article and this is where Ada made history.',
...
addCeleb('Ida Rhodes', '1900', '1986', 'Analysis of Systems of Programming, C-10 Language', 'http://bygeekgirl.com/wp-content/uploads/2016/09/By-Geekgirl-Ida-Rhodes-SIArchives-800x550.jpg', ['As part of the New Deal program created by President Roosevelt, the Mathematical Tables Project was created and Ida Rhodes was taken onboard. She was placed as a planner and supervisor of the Handbook of Mathematical Functions. Seven years later, Ida’s boss sent her to Washington to learn about the creation of an electronic computer which would come to change the course of Ida’s life.',
...
addCeleb('Grace Hopper', '1906', '1992', 'Invented the Compiler and led to COBOL', 'http://bygeekgirl.com/wp-content/uploads/2016/09/By-Geekgirl-Grace-Hopper-Wikipedia-800x550.jpg', ['Grace Hopper was born curious. With her father’s encouragement to be self-sufficient and gain an education, Grace studied math and physics at Vassar and later Yale. In 1934, Grace Hopper was one of the first women to earn a Ph.D. in Mathematics.',
...
addCeleb('Jean Jennings Bartik', '1924', '2011', 'Original Programmer for ENIAC', 'http://bygeekgirl.com/wp-content/uploads/2016/09/By-Geekgirl-Jean-Bartik-Wikipedia-800x550.jpg', ['It was an army advertisement for mathematics graduates that brought Jean to ENIAC. Jean Jennings Bartik became a lead programmer on a tiny team of women working on the first all-electronic digital computer for the army. Jean’s son said she was looking for an adventure and that’s just what she got.',
...
console.dir(celebrities);

// Create a DOM element for each of the objects inside the container.
for (var i = 0; i < celebrities.length; i++){
	var currentCeleb = celebrities[i];

	var domString = '';

	domString += `<article class="celebrity">`;
	domString +=	`<section class="celeb-header">`
	domString +=		`<h2 class="name">${currentCeleb.name}</h2>`;
	domString +=		`<h3 class="dates">(${currentCeleb.birth}-${currentCeleb.death})</h3>`;
	domString +=		`<h3 class="subtitle">${currentCeleb.subTitle}</h3>`;
	domString +=	`</section>`;
	domString +=	`<img src="${currentCeleb.image}" alt="${currentCeleb.name}">`;

	for (var j = 0; j < currentCeleb.bio.length; j++) {
		var currentBio = currentCeleb.bio[j];

		domString +=	`<section class="bio-container">`;
